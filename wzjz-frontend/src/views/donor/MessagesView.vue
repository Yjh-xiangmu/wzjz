  ...
  <div class="chat-container">
    <el-card class="chat-card" shadow="hover" body-style="padding: 0; display: flex; height: calc(100vh - 120px);">
      ...
      <div class="chat-sidebar">
        <div class="sidebar-header">最近沟通</div>
        <div class="contact-list" v-loading="listLoading">
          ...
          >
            <el-avatar class="contact-avatar" :size="40" style="background: #409EFF;">受</el-avatar>
            <div class="contact-info">
              <div class="contact-name">受助方: {{ item.contact_phone }}</div>
            </div>
          </div>
      ...
      <div class="chat-main">
        <template v-if="currentContact">
          <div class="chat-header">
            ...
            <el-button type="primary" plain size="small" icon="List" @click="drawerVisible = true">捐赠记录</el-button>
          </div>
          ...
          <div class="message-area" ref="messageAreaRef">
            <div class="message-list">
              <div v-for="msg in messageList" :key="msg.id" :class="['message-item', msg.senderId === userInfo.id ? 'message-right' : 'message-left']">
                <el-avatar v-if="msg.senderId !== userInfo.id" :size="36" class="msg-avatar" style="background: #409EFF;">受</el-avatar>
    ...
    <el-drawer v-model="drawerVisible" :title="'与 ' + (currentContact?.contact_phone || '') + ' 的捐赠记录'" size="400px">
...
const fetchContactList = async () => {
  ...
  try {
    const res = await request.get(`/api/order/donorContacts?donorId=${userInfo.id}`)
...
watch(drawerVisible, async (newVal) => {
  if (newVal && currentContact.value) {
    ...
    try {
      const res = await request.get(`/api/order/recordsBetween?donorId=${userInfo.id}&recipientId=${currentContact.value.contact_id}`)
      records.value = res || []
    } finally {
